...
def assert_blinker_is_horizontal(soup) -> None:
...
def assert_blinker_is_vertical(soup) -> None:
...
def test_blinker_starts_horizontal(page) -> None:
...
def test_blinker_evolves_to_vertical(page) -> None:
    soup = page('/examples/blinker?generation=2')
    assert_table_is_displayed(soup)
    assert_only_a_blinker_is_present(soup)
    assert_blinker_is_vertical(soup)

def test_blinker_cycles_back_to_horizontal_in_any_odd_generation(page) -> None:
    soup = page('/examples/blinker?generation=3')
    assert_table_is_displayed(soup)
    assert_only_a_blinker_is_present(soup)
    assert_blinker_is_horizontal(soup)
    
def test_blinker_cycles_back_to_vertical_in_any_even_generation(page) -> None:
